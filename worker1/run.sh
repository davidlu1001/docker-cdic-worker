...
#
# chkconfig: 345 99 05
# description: CDIC Worker deamon script
#
...
serviceNameLo="cdic-worker"                                                       # service name with the first letter in lowercase
serviceName="CDIC-Worker"                                                           # service name
serviceUser="cdic"                                                                           # OS user name for the service
serviceGroup="cdic"                                                                         # OS group name for the service
applDir="."                                       # home directory of the service application
serviceUserHome="/home/$serviceUser"                                      # home directory of the service user
serviceLogFile="logs/$serviceNameLo.out"                            # log file for StdOut/StdErr
...
function checkProcessIsOurService {
   ...
   cat /proc/$pid/cmdline | grep "$javaCommandLineKeyword"
...
function startServiceProcess {
   ...
   sudo su -u $serviceUser -c "$javaCommandLine >>$serviceLogFile 2>&1 & echo \$! >$pidFile" || return 1
   sleep 0.1
   pid="$(<$pidFile)"
...
function main {
   ...
   case "$1" in
      ...
      status)                                              # displays the service status
         ...
         ;;
      *)
         echo "Usage: $0 {start|stop|restart|status} worker-name"
         exit 1
         ;;
      esac
   exit $RETVAL
